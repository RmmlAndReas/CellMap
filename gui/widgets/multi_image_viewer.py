...
from qtpy.QtWidgets import QApplication, QGridLayout, QScrollArea, QLabel, QVBoxLayout, QDialog, QStyle, \
    QStyleOptionTitleBar, QMessageBox, QDialogButtonBox, QSizeGrip
# from deprecated_demos.ta.wshed import Wshed
from utils.wshed import wshed
...
class ImgDisplayWindow(QDialog):
    ...
    def resizeEvent(self, event):
        """Override resizeEvent to position the size grip in the bottom-right corner"""
        super().resizeEvent(event)
        self._position_size_grip()
    
    def showEvent(self, event):
        """Override showEvent to position the size grip when the dialog is shown"""
        super().showEvent(event)
        self._position_size_grip()
    
    def _position_size_grip(self):
        """Helper method to position the size grip in the bottom-right corner"""
        if hasattr(self, 'sizeGrip') and self.sizeGrip.isVisible():
            # Position the size grip in the bottom-right corner
            size = self.size()
            grip_size = self.sizeGrip.sizeHint()
            self.sizeGrip.move(size.width() - grip_size.width(), 
                              size.height() - grip_size.height())
    ...
    def initUI(self):
        ...
        # Add size grip for resizing the window
        self.sizeGrip = QSizeGrip(self)
        self.sizeGrip.setVisible(True)
        self.sizeGrip.raise_()  # Make sure it's on top of other widgets
