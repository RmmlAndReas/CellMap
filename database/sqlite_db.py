"""SQLite database operations for CellMap.

This module handles all database operations including:
- Creating and managing SQLite databases
- Storing cell measurements and properties
...
from database.sqlite_tools import get_table_columns
from utils.image_io import Img
from utils.image_utils import has_metadata, _create_dir
from selections.selection import convert_coords_to_IDs
...
def get_table_columns(path_to_db, table_name):
    ...
    """
    return get_table_columns(path_to_db, table_name)
...
def get_type(value):
    ...
    if isinstance(value, list):
        return 'TEXT'
    # Check for all numpy integer types (including uintc, intc, int16, uint16, etc.)
...
def ensure_selected_for_correction_column(db_path):
    """
    Ensures that the 'selected_for_correction' column exists in the 'cell_tracks' table.
    Adds it if it doesn't exist (for migration from older database versions).
    ...
    try:
        ...
        if db.exists('cell_tracks'):
            # Check if column already exists
            columns = db.get_table_column_names_and_types('cell_tracks')
            if 'selected_for_correction' not in columns:
                db.add_column('cell_tracks', 'selected_for_correction', col_type='INTEGER', default_value='0')
                logger.info(f'Added selected_for_correction column to cell_tracks table in {db_path}')
        db.close()
        return True
    except Exception as e:
        logger.error(f'Error ensuring selected_for_correction column: {e}')
        traceback.print_exc()
        return False


def save_track_completeness(completeness_dict, db_path):
    """
    Save track completeness data to database.
    Uses bulk INSERT OR REPLACE for optimal performance.
    
    Args:
        completeness_dict (dict): Dictionary mapping track_id (int) to is_complete (bool)
        db_path (str): Path to the database file (master DB or frame-specific DB)
    
    Returns:
        bool: True if successful, False on error.
    """
    if not completeness_dict:
        logger.warning('No track completeness data to save')
        return False
    
    try:
        db = TAsql(filename_or_connection=db_path)
        
        # Create table if it doesn't exist (WITHOUT ROWID for optimal performance)
        if not db.exists('track_completeness'):
            db.execute_command('''
                CREATE TABLE track_completeness (
                    track_id INTEGER PRIMARY KEY,
                    is_complete INTEGER NOT NULL
                ) WITHOUT ROWID
            ''')
            logger.info(f'Created track_completeness table in {db_path}')
        
        # Prepare data for bulk insert
        data = [(track_id, 1 if is_complete else 0) 
                for track_id, is_complete in completeness_dict.items()]
        
        # Bulk insert using executemany for optimal performance
        db.cur.executemany(
            'INSERT OR REPLACE INTO track_completeness VALUES (?, ?)',
            data
        )
        db.con.commit()
        
        db.close()
        logger.info(f'Saved track completeness for {len(completeness_dict)} tracks to {db_path}')
        return True
    except Exception as e:
        logger.error(f'Error saving track completeness to {db_path}: {e}')
        traceback.print_exc()
        return False


def load_track_completeness(db_path):
    """
    Load track completeness data from database.
    
    Args:
        db_path (str): Path to the database file (master DB or frame-specific DB)
    
    Returns:
        dict: Dictionary mapping track_id (int) to is_complete (bool), or empty dict if not found/error.
    """
    try:
        db = TAsql(filename_or_connection=db_path)
        ...
        # Single query to load all data - very fast
        results = db.run_SQL_command_and_get_results(
            'SELECT track_id, is_complete FROM track_completeness'
        )
        
        db.close()
        
        if results:
            completeness = {int(row[0]): bool(row[1]) for row in results}
            logger.info(f'Loaded track completeness for {len(completeness)} tracks from {db_path}')
            return completeness
        else:
            return {}
    except Exception as e:
        logger.warning(f'Error loading track completeness from {db_path}: {e}')
        traceback.print_exc()
        return {}


def save_track_confidence(confidence_dict, frame_idx, db_path):
    """
    Save track confidence data to database.
    Uses bulk INSERT OR REPLACE for optimal performance.
    
    Args:
        confidence_dict (dict): Dictionary mapping track_id (int) to confidence score (float, 0.0-1.0)
        frame_idx (int): Target frame index (0-based) - confidence is for transition to this frame
        db_path (str): Path to the database file (master DB)
    
    Returns:
        bool: True if successful, False on error.
    """
    if not confidence_dict:
        logger.warning('No track confidence data to save')
        return False
    ...
    try:
        ...
        # Create table if it doesn't exist (WITHOUT ROWID for optimal performance)
        if not db.exists('track_confidence'):
            db.execute_command('''
                CREATE TABLE track_confidence (
                    track_id INTEGER,
                    frame_idx INTEGER,
            ...
            ''')
            logger.info(f'Created track_confidence table in {db_path}')
        
        # Prepare data for bulk insert
        data = [(track_id, frame_idx, float(confidence)) 
                for track_id, confidence in confidence_dict.items()]
        
        # Bulk insert using executemany for optimal performance
        db.cur.executemany(
            'INSERT OR REPLACE INTO track_confidence VALUES (?, ?, ?)',
            data
        )
        db.con.commit()
        
        db.close()
        logger.info(f'Saved track confidence for {len(confidence_dict)} tracks at frame {frame_idx} to {db_path}')
        return True
    except Exception as e:
        logger.error(f'Error saving track confidence to {db_path}: {e}')
        traceback.print_exc()
        return False


def load_track_confidence(track_id, db_path):
    """
    Load track confidence data from database for a specific track.
    
    Args:
        track_id (int): Track ID to load confidence for
        db_path (str): Path to the database file (master DB)
    
    Returns:
        dict: Dictionary mapping frame_idx (int) to confidence_score (float), sorted by frame_idx.
              Returns empty dict if track not found or error.
    """
    try:
        db = TAsql(filename_or_connection=db_path)
        ...
        # Query for all confidence scores for this track using parameterized query
        # Convert track_id to int to ensure type matching
        track_id_int = int(track_id) if track_id is not None else None
        db.cur.execute(
            'SELECT frame_idx, confidence FROM track_confidence WHERE track_id = ? ORDER BY frame_idx',
            (track_id_int,)
        )
        ...
        if results:
            confidence = {int(row[0]): float(row[1]) for row in results}
            logger.debug(f'Loaded track confidence for track {track_id}: {len(confidence)} frames from {db_path}')
            return confidence
        else:
            return {}
    except Exception as e:
        logger.warning(f'Error loading track confidence from {db_path}: {e}')
        traceback.print_exc()
        return {}


def get_master_db_path(file_list):
    """
    Get the Master.db path in the main folder. Creates the directory if needed.
    
    Args:
        file_list (list): List of file paths
    
    Returns:
        str: Path to Master.db in the main folder, or None on error
    """
    if not file_list:
        return None
    ...
    try:
        ...
        ta_path = smart_name_parser(first_file, ordered_output='TA')
        # TA path is like: /path/to/main/Image0001
        # Main folder is the parent: /path/to/main
        main_folder = os.path.dirname(ta_path)
        ...
        if not os.path.exists(main_folder):
            try:
                ...
                logger.error(f'Could not create main folder {main_folder}: {e}')
                return None
        
        # Master.db path in the main folder
        master_db_path = os.path.join(main_folder, 'Master.db')
                    ...
                    if force_track_cells_db_update:
                        ...
                        if not force_update:
                            ...
                            if force_update:
                                ...
                                dbHandler.execute_command('CREATE TABLE cell_tracks AS SELECT local_id as local_id, CAST(NULL AS INTEGER) AS track_id, CAST(0 AS INTEGER) AS selected_for_correction FROM cells_2D')
