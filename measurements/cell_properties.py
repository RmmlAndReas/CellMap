...
This module computes various cell properties from segmented images including:
- 2D measurements: area, perimeter, aspect ratio, orientation
- 3D measurements: volume, surface area, height maps
- Nematic measurements: cell alignment and orientation fields
- Vertex and bond detection: cell-cell junction analysis

Results are stored in SQLite databases for further analysis.
"""
...
from utils.image_utils import pad_border_xy
from skimage.measure import label, regionprops
import numpy as np
import traceback
from measurements.nematic import compute_stretch_nematic
from utils.early_stopper import early_stop
from utils.list_utils import loadlist
from database.sqlite_db import TAsql, get_property
from measurements.nematic_measurements import compute_polarity_nematic
...
def TAMeasurements(file_or_list, __forced_orig=None, __forced_cells=None, __forced_heightmap=None,
    ...
    for bond in rps_bonds:
        ...
        if len(vertices_ids) > 2:
            try:
                # logger.error(                'error: wrong number of vertices --> please report this error' + str(vertices_ids))
                ...
                traceback.print_exc()
                if __DEBUG__:
                    print('error correcting the number of vertices of the bond --> please report this error')
                    logger.error(
                        'error correcting the number of vertices of the bond --> please report this error')
    ...
    for vertex in rps_verts:
        ...
        if vertex.area >= 2:
            ...
            if vertex.area > 2:  # in fact that is easy I could split it into n closest pairs of points --> that should work, but first let's see if it is possible or not ???
                # logger.error('error: up to '+str(vertex.area)+ 'vertices can be adjacent to each other - please report this bug'+str( vertex.coords))
                # sys.exit(0)
            ...
            for lst in pairs:
                ...
                if len(cells_around_bond) > 2:
                    print('pairs of vertices have too many cells in common, please report this error')
    ...
    for cell in rps:
        ...
        #     logger.error(
        #         'error: wrong number of vertices --> please report this error' + str(vertices_indices))
        #
        # # print('neo vertices', vertices_indices) # this gives me the indices of the vertices of the bonds --> see how I can associate them with a cell at some point maybe if necessary
