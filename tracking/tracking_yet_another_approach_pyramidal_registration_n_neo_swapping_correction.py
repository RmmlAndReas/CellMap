"""Pyramidal registration and swapping correction for cell tracking.

This module implements advanced tracking with pyramidal image registration
and automatic detection/correction of cell ID swapping errors.
"""
...
from tracking.registration import get_pyramidal_registration
from tracking.tools import assign_random_ID_to_missing_cells, first_image_tracking, get_list_of_files, \
    get_input_files_and_output_folders, get_TA_file, get_mask_file, smart_name_parser
...
from tracking.tracking_error_detector_and_fixer import find_vertices, associate_cell_to_its_neighbors2, \
...
def track_cells_dynamic_tissue(path, channel=None, PYRAMIDAL_DEPTH=3, MAX_ITER=15, progress_callback=None):
    ...
    if len(images_to_analyze)==1:
        ...
        mask_t0 = Img(get_mask_file(filename0_without_ext))
        if mask_t0.has_c():
            mask_t0 = mask_t0[..., 0]
    ...
    for l in range(len(images_to_analyze) - 1):
        ...
        if not os.path.exists(mask_file_path):
            logger.error(f"Could not find mask file (tried handCorrection.tif and outlines.tif): {mask_file_path}")
            continue
        ...
        if not os.path.exists(mask_t1_path):
            logger.error(f"Could not find mask file (tried handCorrection.tif and outlines.tif): {mask_t1_path}")
            continue
        ...
        # Initialize corrected_track in case the loop doesn't execute or FIX_SWAPS is False
        corrected_track = track_t_cur
        
        # Initialize confidence score dict to store final scores after optimization
        ...
        for lll in range(MAX_ITER):
            ...
            if FIX_SWAPS:
                ...
                # Store final confidence scores after optimization (will be overwritten each iteration)
        ...
        # Save tracking confidence scores to master database (AFTER optimization loop completes)
        # Only save if we have a previous frame (l > 0) and confidence scores were computed
        if l > 0 and final_confidence_scores:
            try:
                # Get all unique track IDs in the current frame
                unique_tracks = np.unique(track_t_cur)
                unique_tracks = unique_tracks[unique_tracks != 0xFFFFFF]  # Remove background
                
                # Build confidence dictionary for all tracks in current frame
                # Use scores from optimization, or 0.0 for tracks not in the score dict
                final_confidence = {}
                for track_id in unique_tracks:
                    if track_id in final_confidence_scores:
                        final_confidence[track_id] = final_confidence_scores[track_id]
                    else:
                        # Track exists in current frame but wasn't in previous frame
                        # This could be a new track or a track that appeared
                        # Set confidence to 0.0 (no match with previous frame)
                        final_confidence[track_id] = 0.0
                
                # Save confidence scores for frame transition l -> l+1 (stored with target frame l+1)
                if final_confidence:
